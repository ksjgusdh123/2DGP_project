...
mode = {'Run': run_track_mode, 'Swim': swimming_mode, 'long-jump': long_jump_mode, 'Shooting': shooting_mode}
now_map = None
scores = []
        ...
        if event.type == SDL_QUIT:
            ...
            if now_map == 'long-jump' and long_jump_mode.jump_chance > 0:
                ...
                if mode[now_map].player.next_map == 'swim':
                    mode[now_map].delete_object_all()
...
def init():
    ...
    fill_records()
    if now_map == 'long-jump' or now_map == 'Shooting':
...
def jump_records_down_sort_print():
    for i in range(0, 3 + 1):
        ...
        font.draw(300, 400 - 66 * i, f"{first_records[i][0]:>3.0f}cm   {first_records[i][1]:>3.0f}cm", (0, 0, 0))
...
def score_plus():
    if now_map == 'Shooting':
        for i in range(0, 3 + 1):
            if records[i][2] == mode[now_map].player.character_id:
                mode[now_map].player.score += 10 - 3 * i

            for j in range(3):
                if mode[now_map].ai[j].ch_id == records[i][2]:
